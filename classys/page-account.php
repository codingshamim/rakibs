...
$selUser = "select * from users where uniqueId = '$userId'";
...
if(isset($_POST['upAcc'])){
  ...
  $updateUser = "UPDATE `users` SET `fName` = '$fName', `email` = '$email', `phone` = '$phone' WHERE `users`.`uniqueId` = '$userId'";
  if(mysqli_query($conn,$updateUser)){
    ...
    $succMsg = "User Updated Successfully";
      ...
      if(isset($_POST['upPass'])){
        $orginalpass =  $users['pass'];
        $olPass = mysqli_real_escape_string($conn,md5($_POST['olPass']));
        $nPass = mysqli_real_escape_string($conn,md5($_POST['nPass']));
        $cPass = mysqli_real_escape_string($conn,md5($_POST['cPass']));
        // now update password sql and query
        if($orginalpass == $olPass){
          if($nPass == $cPass){
            $updatePassSql = "UPDATE `users` SET `pass` = '$nPass', `repass` = '$cPass' WHERE `users`.`uniqueId` = '$userId'";
            ...
            $err = true;
            $errMsg = "New Password And Confirm Password Are Not Matched";
          }
       
        }else{
          $err = true;
          $errMsg = "Old Password Is Not Matched";
        }
       
      }

// count the address
$countSql = "SELECT * FROM `information` where user ='$userId'";
$countQuery = mysqli_query($conn,$countSql);
$counters = mysqli_num_rows($countQuery);

$address = mysqli_fetch_assoc($countQuery);

        // update the shipping address


        if(isset($_POST['addMake'])){
          $fnames = mysqli_real_escape_string($conn,$_POST['fnames']);
          $addOne = mysqli_real_escape_string($conn,$_POST['addOne']);
          $addTwo = mysqli_real_escape_string($conn,$_POST['addTwo']);
          $method = mysqli_real_escape_string($conn,$_POST['method']);
          $city = mysqli_real_escape_string($conn,$_POST['city']);
          $zipCode = mysqli_real_escape_string($conn,$_POST['zipCode']);
          $phoneNBs = mysqli_real_escape_string($conn,$_POST['phoneNBs']);
          $desShip = mysqli_real_escape_string($conn,$_POST['desShip']);
          // new address insert
          if($counters >= 2){
           $err = true;
           $errMsg = "You Can Add Only Two Address";
          }else{
        // note : // update address working
            $insertSql = "INSERT INTO `information` ( `fNames`,`addOne`, `addTwo`, `method`, `city`, `postCode`, `phone`, `fullAdd`, `user`) VALUES ('$fnames', '$addOne', '$addTwo', '$method', '$city', '$zipCode', '$phoneNBs', '$desShip','$userId')";
            if(mysqli_query($conn,$insertSql)){
              $succ = true;
              $succMsg = "Shiping Address Created Successfully";
            }
        ...
        <div class="container box-account-template">
          ...
          <div class="box-tabs mb-100">
            <ul class="nav nav-tabs nav-tabs-account" role="tablist">
              <li><a class="" href="#tab-notification" data-bs-toggle="tab" role="tab" aria-controls="tab-notification" aria-selected="false">Notification</a></li>
              ...
              <li><a href="#tab-setting" data-bs-toggle="tab" role="tab" aria-controls="tab-setting" aria-selected="true" class="active">Setting</a></li>
            </ul>
              ...
              <div class="tab-pane fade active show" id="tab-setting" role="tabpanel" aria-labelledby="tab-setting">
                <div class="row">
                  <div class="col-lg-6 mb-20">
                    <form action="page-account.php" method="post">
                      <div class="row">
                        <div class="col-lg-12 mb-20">
                          <h5 class="font-md-bold color-brand-3 text-sm-start text-center">Personal information</h5>
                        </div>
                        <form action="page-account.php" method="post">
                        <div class="col-lg-12">
                          <div class="form-group">
                            <input class="form-control font-sm" type="text" value="<?php echo $users['fName'] ?>" name="fName">
                          </div>
                        </div>
                      
                        <div class="col-lg-12">
                          <div class="form-group">
                            <input type="number" class="form-control font-sm" type="text" value="<?php echo $users['phone'] ?>" name="phone">
                          </div>
                        </div>
                        <div class="col-lg-12">
                          <div class="form-group">
                            <input class="form-control font-sm" type="text" value="<?php echo $users['email'] ?>" name="email">
                          </div>
                          <button type="submit" class="btn btn-brand-3" name="upAcc">Save Changes</button>
                        </div>

                      
                        </form>
                       <h6 class="mt-2">Change Your Password</h6>
                       <div class="col-lg-12 mt-3">
                        <form action="page-account.php" method="post">
                          <div class="form-group">
                            <input class="form-control font-sm" type="text" placeholder="Old Password" name="olPass" required>
                          </div>
                        </div>
                        <div class="col-lg-12 mt-3">
                          <div class="form-group">
                            <input class="form-control font-sm" type="text" placeholder="New Password" name="nPass" required>
                          </div>
                        </div>
                        <div class="col-lg-12 mt-3">
                          <div class="form-group">
                            <input class="form-control font-sm" type="text" placeholder="Confirm Password" name="cPass" required>
                          </div>
                          <button type="submit" class="btn btn-brand-3" name="upPass">Save Changes</button>
                        </div>
                        </form>
                        <div class="col-lg-12">
                      ...
                      <form action="page-account.php" method="post">
                        <div class="col-lg-12">
                          <div class="form-group">
                            <input class="form-control font-sm" type="text" placeholder="Full name*" name="fnames">
                          </div>
                        </div>
                      
                        <div class="col-lg-12">
                          <div class="form-group">
                            <input class="form-control font-sm" type="text" placeholder="Address 1*" name="addOne">
                          </div>
                        </div>
                        <div class="col-lg-12">
                          <div class="form-group">
                            <input class="form-control font-sm" type="text" placeholder="Address 2*" name="addTwo">
                          </div>
                        </div>
                        <div class="col-lg-6">
                          <div class="form-group">
                            <select class="form-control font-sm select-style1 color-gray-700" name="method">
                        ...
                        <div class="col-lg-6">
                          <div class="form-group">
                            <input class="form-control font-sm" type="text" placeholder="City*" name="city">
                          </div>
                        </div>
                        <div class="col-lg-12">
                          <div class="form-group">
                            <input class="form-control font-sm" type="number" placeholder="PostCode / ZIP*" name="zipCode">
                          </div>
                        </div>
                      
                        <div class="col-lg-12">
                          <div class="form-group">
                            <input class="form-control font-sm" type="text" placeholder="Phone*" name="phoneNBs">
                          </div>
                        </div>
                        <div class="col-lg-12">
                          <div class="form-group mb-0">
                            <textarea class="form-control font-sm" placeholder="Full Address" rows="5" name="desShip"></textarea>
                          </div>
                        </div>
                  
                        <div class="col-lg-12">
                          <div class="form-group mt-20">
                          <button type="submit" class="btn btn-brand-3" name="addMake">Save Changes</button>
                          </div>
                        </div>
                      </div>
                    </form>
                  ...
                  <div class="col-lg-5 mb-20">
                    <div class="mt-40">
                      <h4 class="mb-10"><?= $users['fName'] ?></h4>
                      <div class="mb-10">
                        <p class="font-sm color-brand-3 font-medium">Personal Information:</p><span class="font-sm color-gray-500 font-medium"><?php echo $users['fName'] ?>, <?php echo $users['phone'] ?>, <?php echo $users['email'] ?></span>
                      </div>
                      ...
                      <div class="mb-10">
                        <p class="font-sm color-brand-3 font-medium">Delivery address:</p><span class="font-sm color-gray-500 font-medium"><?php echo $address['addOne'] ?>, <?php echo $address['addTwo'] ?>, <?php echo $address['city'] ?>, <?php echo $address['postCode'] ?> </span>
                      </div>
                      
                      <div class="mb-10">
                        <p class="font-sm color-brand-3 font-medium">Phone Number:</p><span class="font-sm color-gray-500 font-medium"><?php echo $address['phone'] ?></span>
                      </div>
              ...
              <div class="row">
                ...
                <div class="col-w-1">
                  ...
                  <ul class="list-checkbox">
                    ...
                    <li>
                      <label class="cb-container">
                        <input type="checkbox" checked="checked"><span class="text-small">1–3 business days</span><span class="checkmark"></span>
                      </label>
                    </li>
                    <li>
                      <label class="cb-container">
